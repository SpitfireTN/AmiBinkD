...
/* w9x_workaround_sleep: 1000000 = 1 sec, 10000 = 10 ms */
  ...
  #define TCPERR_WOULDBLOCK EWOULDBLOCK
  #define TCPERR_AGAIN EAGAIN
  #define sock_init() amiga_socket_init()
  #define sock_deinit() amiga_socket_cleanup()
  /* bsdsocket.library handles are not AmigaDOS file handles - closing them
   * with regular close() (or DOS Close()) is wrong. Must go through
   * bsdsocket.library's own CloseSocket(). */
  #define soclose(h) CloseSocket(h)
#else
  #include <errno.h>
  #define ReleaseErrorList()
