...
/* chld()/sighandler() are real fork/SIGCHLD/SIGHUP machinery - common.h's
 * check_child() macro already no-ops without HAVE_FORK (never calls
 * chld() at all), so neither of these is reachable in a synchronous,
 * non-forking AMIGA build; no point linking them. */
#ifdef HAVE_FORK
...
void usage (void)
{
  ...
  printf ("  -P node  poll a node ('-P ALL' polls every node in the config)\n"
	  ...
	  "Modern port by Gary (Spitfire) McCulloch. SpitfireTN Entertainment.\n"
	  );
...
#endif
{
  ...
  Log (4, "BEGIN, " AMIBINKD_NAME " " AMIBINKD_VER " " MYNAME "/" MYVER);
#else
