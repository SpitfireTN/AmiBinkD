...
#define SHELLOPT "/c"
#elif defined(AMIGA)
  /* SHELL/SHELL_META/SHELLOPT are a Unix-shell-quoting concept that
   * doesn't apply here - run() below uses SystemTagList() directly
   * (AmigaDOS's own command-line interpreter), and run3()'s pipe-based
   * stdin/stdout/stderr redirection is stubbed out (see run3() below),
   * so neither of these three is actually referenced on AMIGA. */
#else
...
#elif defined(AMIGA)
  /* SystemTagList() is AmigaDOS's own "run this command line through the
   * CLI interpreter and wait for it" call - the direct native equivalent
   * of system(), no ixemul needed. */
  Log (3, "executing `%s'", cmd);
  rc = SystemTagList((CONST_STRPTR)cmd, NULL);
  Log (3, "rc=%i", rc);
#else
  Log (3, "executing `%s'", cmd);
  Log (3, "rc=%i", (rc=system (cmd)));
...
{
#ifdef AMIGA
  /* Not implemented on Amiga: bsdsocket.library sockets and AmigaDOS
   * file handles (which is what a real PIPE: implementation would need
   * to hand back through *in/*out/*err) aren't interchangeable the way
   * Unix fds are without ixemul's unification layer - resolving that is
   * its own project, not something to bolt on here. Only caller today
   * is client.c's external-transport-via-pipe feature, which is not a
   * core binkp/FTN feature; run() above (used by srif.c's mail-event
   * hooks, the common case) works normally via SystemTagList(). */
  UNUSED_ARG(in);
  UNUSED_ARG(out);
  UNUSED_ARG(err);
  Log(1, "run3: external command with piped I/O is not supported on Amiga: `%s'", cmd);
  return -1;
#else
  int pid;
