 ...
 * deliberately avoids (see AMIGA branch in branch() below - it runs
 * synchronously instead, same as the DOS build). The old ixemul-era
 * ix_vfork()/vfork_setup_child()/ix_vfork_resume() declarations that used
 * to live here are gone on purpose. */
...
#if defined(DOS) || defined(DEBUGCHILD) || defined(AMIGA)
  /* No fork()/threads here (AMIGA: deliberately not using ixemul's
   * ix_vfork() - see comment above). Run synchronously instead, same
   * as the DOS build: one binkp session at a time, no concurrency. */
  rc = 0;
  F (arg);
#endif
