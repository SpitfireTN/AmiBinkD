/* client.c – AmigaOS 3.x–safe client module */
...
void rel_grow_handles(int nh)
{
    ...
    if (DosSetRelMaxFH(&addfh, &curmaxfh))
#endif
        Log(1, "Cannot grow handles to %ld (now %ld): %s",
            curmaxfh, addfh, strerror(errno));
    else
...
static int call0(FTN_NODE *node, BINKD_CONFIG *config)
{
...
#ifdef HTTPS
    use_proxy = (node->NP_flag != NP_ON) &&
                (!node->pipe || !node->pipe[0]) &&
                (proxy[0] || socks[0]);
    if (use_proxy)
    {
        char *sp, *sport;
        ...
        {
            Log(2, "Port %s not found, try default %d",
                sport, proxy[0] ? 3128 : 1080);
            aiErr = getaddrinfo(host,
                                proxy[0] ? "3128" : "1080",
                                &hints, &aiProxyHead);
        }
        if (aiErr != 0)
        {
            Log(1, "%s host %s not found",
                proxy[0] ? "Proxy" : "Socks", host);
#ifdef WITH_PERL
            xfree(hosts);
            xfree(proxy);
            xfree(socks);
    ...
    for (i = 1;
         ...
         (rc = get_host_and_port(i, host, port, hosts, &node->fa, config)) != -1;
         ++i)
    {
        if (rc == 0)
        {
        ...
        {
            char *cmdline = xstrdup(node->pipe);
            ...
            if (!binkd_exit)
            {
                Log(1, "connection to %s failed", szDestAddr);
            }
            sockfd = INVALID_SOCKET;
            continue;
        }
        ...
        for (ai = aiHead;
             ai != NULL && sockfd == INVALID_SOCKET;
             ai = ai->ai_next)
        {
            for (j = 0; j < NO_INVALID_ADDRESSES; j++)
            {
                if (0 == sockaddr_cmp_addr(ai->ai_addr,
                                           (struct sockaddr *)&invalidAddresses[j]))
                {
#if defined(AF_INET6) && defined(_SS_MAXSIZE)
...
#else
                    const int l = (int)sizeof(struct sockaddr_in);
#endif
                    rc = getnameinfo((struct sockaddr *)&invalidAddresses[j],
                                     l,
            ...
            if (use_proxy)
            {
                ...
                if (strcmp(port, config->oport) == 0)
                    Log(4, "trying %s via %s %s:%s...",
                        host,
                        proxy[0] ? "proxy" : "socks",
                        addrbuf, servbuf);
                else
                    Log(4, "trying %s:%s via %s %s:%s...",
                        host, port,
                        proxy[0] ? "proxy" : "socks",
                        addrbuf, servbuf);
            }
            ...
            if (config->bindaddr[0])
            {
                ...
                if ((aiErr = getaddrinfo(config->bindaddr, NULL,
                                         &src_hints, &src_ai)) == 0)
                {
                ...
                else if (aiErr == EAI_FAMILY)
                {
                    continue;
                }
                else
                {
                    Log(2, "bind -- getaddrinfo: %s (%d)",
                        gai_strerror(aiErr), aiErr);
                }
            }
            ...
            if (!binkd_exit)
            {
                Log(1, "connection to %s failed: %s",
                    szDestAddr, save_err);
    ...
    if (pid != -1)
    {
        del_socket(sock_out);
        close(sock_out);
#if defined(HAVE_WAITPID) && !defined(AMIGA)
        ...
        {
            if (WIFSIGNALED(rc))
                Log(2, "process %u exited by signal %u",
                    pid, WTERMSIG(rc));
            else
...
static void call(void *arg)
{
    struct call_args *a = (struct call_args *)arg;
    char              szDestAddr[FTN_ADDR_SZ + 1];
#if defined(WITH_PERL) && defined(HAVE_THREADS)
    void *cperl;
#endif
